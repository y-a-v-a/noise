...
for (let i = 0; i < 100; i++) {
  let noise = ImprovedNoise.noise(Math.random() * Math.PI * 500,Math.random() * 11 * Math.PI * 500,Math.random() * 6);
...
gd.createTrueColor(500, 500, (error, img) => {
  for (let x = 0; x < 500; x++) {
    for (let y = 0; y < 500; y++) {
      ...
      let noiseR = (.5 + .25 * noise / (1 << 16));
      let noiseQ = (.5 + .25 * noise);
      //console.log(1000000*(.25 * noise / (1 << 16)));

      //let noise = ImprovedNoise.noise(x, y, Math.random());
      let channelValueR = 127 + 1000000*(.25 * noise / (1 << 16));
      let channelValueG = 127 + 1000000*(.25 * noise / (1 << 16));
      let channelValueB = 127 + 1000000*(.25 * noise / (1 << 16));
      //let channelValue = Math.random() * 127 + 127;
      let color = gd.trueColor(~~channelValueR,~~channelValueG,~~channelValueB);
      //console.log(color);
